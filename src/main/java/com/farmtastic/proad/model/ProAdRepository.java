...
//			  List<ProAdVO> findAllByFarmer(@Param("fmemId") Integer fmemId);
	...
	// 活動首頁輪播圖片
	@Query(" SELECT p.proAdId FROM ProAdVO p WHERE p.proAdRevStat = 5 AND p.proAdLaunStat = 1 AND p.proAdStart <= CURRENT_DATE AND p.proAdEnd   >= CURRENT_DATE ORDER BY p.proAdStart DESC")

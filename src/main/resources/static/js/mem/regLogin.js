...
switchToDisplay(btnLogin, btnReg, login, register);

function switchToDisplay(aSwitch, bSwitch, aDisplay, bDisplay){
    aSwitch.addEventListener('click', () => {
        aDisplay.style.display = "block";
        bDisplay.style.display = "none";

        aSwitch.classList.add('active-btn');
        bSwitch.classList.remove('active-btn');
    })
    bSwitch.addEventListener('click', () => {
        bDisplay.style.display = "block";
        aDisplay.style.display = "none";

        bSwitch.classList.add('active-btn');
        aSwitch.classList.remove('active-btn');
    })
}


//----------------- 有錯誤訊息時 頁籤保持在原本位置(登入/註冊)

window.addEventListener("load", loadOriginalTab);

function loadOriginalTab(){
	let activeTab = document.body.dataset.activeTab || 'login';
